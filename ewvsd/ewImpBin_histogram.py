...
#           if np.isnan(EW[j]) == False:
#               print EW[j]
#               EW[j] = np.log10(EW[j])
#           else:
#               EW[j] = 0.0
 
        # Take the log of the EW array
        EW = np.log10(EW)
        print np.isnan(EW)
...
#        ax.set_xlim([0.0, 1.5])
        ax.set_ylim([ewmin, ewmax[i]])
#        ax.text(-1, 7, labels[i])
        if i==0:
            ax.set_title('dwSN', size=12)
            ax.set_ylabel('EW$_{HI}$ [$\AA$] ', multialignment='center')
        elif i==1:
            ax.set_ylabel('EW$_{MgII}$ [$\AA$] ', multialignment='center')
        elif i==2:
            ax.set_ylabel('EW$_{CIV}$ [$\AA$] ', multialignment='center')
        elif i==3:
            ax.set_ylabel('EW$_{OVI}$ [$\AA$] ', multialignment='center')
...
#    fig.text(0.05, 0.15, 'OVI', rotation='vertical')
    ...
    plt.tight_layout()
    s = file_loc+'ewImp_histo.master_bulk.eps'
    plt.savefig(s, bbox_inches='tight')

    s = file_loc+'ewImp_histo.master_bulk.pdf'
    plt.savefig(s, bbox_inches='tight')
