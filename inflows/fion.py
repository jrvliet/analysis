...
header = [tempHeader,denseHeader,ions,fields]
    ...
    for ion in ions:
        ...
        for j in range(numTbins):
            ...
            for k in range(numNbins):
                ...
                denseBins = (df['nH']>10**loN) & (df['nH']<10**hiN)
                fil = df[spaceInds & tempBins & denseBins]

                fil['mass'] = (fil['nH']*u2g*pc2cm**3/g2M)*fil['cell_size']**3
    
                results[tempLabel,denseLabel,ion,fields[0]].ix[i] = np.average(fil['fIon'],
                                                                    weights=fil['mass'])
                results[tempLabel,denseLabel,ion,fields[1]].ix[i] = fil['fIon'].std()

s = 'fIon.h5'
results.to_hdf(s,'data',mode='w',format='fixed')
s = 'fIon_utf.csv'
results.to_csv(s,encoding='utf-8')
s = 'fIon_ascii.csv'
results.to_csv(s,encoding='ascii')
