...
for galNum in halo_list:
    ...
    print('Galaxy: vela2b-{0:d}'.format(galNum))
    fig, ((ax1, ax2),(ax3,ax4)) = plt.subplots(2,2, figsize=(10,10))
    axes = (ax1, ax2, ax3, ax4)

    for ion, ax in zip(ions, axes):
        
        print('\tIon: {0:s}'.format(ion))
        ...
        cbar.ax.set_ylabel('Sum EW', rotation=270, fontsize=12)
    ...
    fig.tight_layout()
    fig.savefig('pahist_vela2b-{0:d}_ew_{1:s}_ewcut{2:d}mA_{3:s}.pdf'.format(galNum, statname, int(1000*ewcut), statname), bbox_inches='tight')
